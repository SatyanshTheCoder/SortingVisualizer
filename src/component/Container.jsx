...
export const Container = ({
  ...
  return (
    <div className={`flex flex-col items-center  ${isOn?"h-96":"flex-grow"} ${dark?'text-white':"text-black"}`}>
      <main className="flex flex-col flex-grow pt-7 min-w-[300px] max-h-[400px] pb-11 mt-2 max-w-full px-4">
        <div
          ...
          style={{ boxShadow: `${dark?'0 0px 4px rgba(255, 255, 255, 0.4)':"0 0 4px rgba(0, 0, 0, 0.4)"}` }}
        >
          <header className="flex justify-between">
            <h2 className="font-bold text-lg">{formattedPath} Sort</h2>
            <h2>Time: <b>{Math.floor(time/100)}</b></h2>
          </header>
          ...
          <footer className="flex justify-between">
            <h2>Swaps:<b>{swaps}</b></h2>
            <h2>Comparison: <b>{comp}</b> </h2>
          </footer>
        </div>
      </main>
    </div>
